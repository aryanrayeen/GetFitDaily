...
const ProgressTracker = () => {
  ...
  const addWeightEntry = async () => {
    if (currentWeight) {
      setLoading(true);
      try {
        const response = await api.post('/biometrics/weight', {
          weight: parseFloat(currentWeight)
        });
        
        if (response.data) {
          // Update user info with new weight
          setUserInfo(prev => ({ ...prev, currentWeight: parseFloat(currentWeight) }));
        ...
        const errorMessage = error.response?.data?.message || 'Failed to add weight entry. Please try again.';
        alert(errorMessage);
      } finally {
        setLoading(false);
      }
  ...
  const addWaistEntry = async () => {
    if (currentWaist) {
      setLoading(true);
      try {
        const response = await api.post('/biometrics/waist', {
          waistMeasurement: parseFloat(currentWaist)
        });
        
        if (response.data) {
          // Refresh waist history
          await fetchWaistHistory();
          
          setCurrentWaist('');
        }
      } catch (error) {
        console.error('Error adding waist entry:', error);
        const errorMessage = error.response?.data?.message || 'Failed to add waist measurement. Please try again.';
        alert(errorMessage);
      } finally {
        setLoading(false);
      }
  ...
  const addBodyFatEntry = async () => {
    if (currentBodyFat) {
      setLoading(true);
      try {
        const response = await api.post('/biometrics/bodyfat', {
          bodyFatPercentage: parseFloat(currentBodyFat)
        });
        
        if (response.data) {
          // Refresh body fat history
        ...
        const errorMessage = error.response?.data?.message || 'Failed to add body fat entry. Please try again.';
        alert(errorMessage);
      } finally {
        setLoading(false);
      }
